...
public final class Matrix<T> extends AbstractMatrix<T[], List<T>, Stream<T>, Stream<Stream<T>>, Matrix<T>> {
     ...
     */
    public static <T> Matrix<T> repeat(final Class<T> elementClass, final T element, final int len) throws NullPointerException {
     ...
     * @throws E the e
     */
    public <R, E extends Exception> Matrix<R> map(final Class<R> cls, final Throwables.Function<? super T, R, E> func) throws E {
    ...
    public <B, R, E extends Exception> Matrix<R> zipWith(final Class<R> targetElementType, final Matrix<B> matrixB,
        ...
        N.checkArgument(rows == matrixB.rows && cols == matrixB.cols, "Can't zip two or more matrices which don't have same shape");

        final R[][] result = N.newArray(N.newArray(targetElementType, 0).getClass(), rows);

        for (int i = 0; i < rows; i++) {
            result[i] = N.newArray(targetElementType, cols);
        }
