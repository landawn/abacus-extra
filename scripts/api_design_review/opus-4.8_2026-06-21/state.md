...
- Arrays = sealed; permits Arrays.f. Nested public final classes: f (extends Arrays), ff, fff. Main Arrays class = static 2D/3D array utilities organized as REPEATING PER-TYPE BLOCKS (boolean/char/byte/short/int/long/float/double/generic), each block = {println, updateAll, replaceIf, reshape, flatten, mutateAsFlatArray, zip, elementCount, minRowLength, maxRowLength}. Separate toX conversion block (16449-19902). f/ff/fff = functional 1D/2D/3D helpers (map/mapToX/zip/updateAll/etc.). Arrays template-mirrored ==> cross-type consistency is primary axis.
...
| AR2 | Arrays: reshape, flatten, mutateAsFlatArray, elementCount, min/maxRowLength, println (structural) | done |
